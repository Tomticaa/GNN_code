...
class GraphConvolution(nn.Module):  # 定义卷积层: 定义输入输出特征，可训练的权重以及偏置并对其进行初始化
    def __init__(self, input_dim, output_dim, use_bias=True):
        ...
        self.weight = nn.Parameter(
            torch.FloatTensor(input_dim, output_dim))  # 创建权重矩阵，应用Parameter方法便于执行梯度的简便更新，大小为输入特征维度*输出特征维度
        if self.use_bias:  # 在该层是否使用偏置？增加模型的灵活性
            self.bias = nn.Parameter(torch.FloatTensor(output_dim))  # 初始化输出特征维度*1的偏置
        else:  #
            self.register_parameter('bias',
                                    None)  # 这行代码显式地在网络的参数字典中注册一个名为 bias 的参数，但将其设置为 None。这样做是为了确保即使在不使用偏置的情况下，网络的其他部分仍然可以正常访问 bias 参数（尽管它是 None），从而保持代码的一致性和防止出错。
    ...
    def forward(self, adjacent, input_feature):  # 将特征表示与邻接矩阵作为输入，进行迭代计算，这里的邻接矩阵已经在数据集部分处理好的归一化矩阵格式；
        support = torch.mm(input_feature, self.weight)  # 计算： H * W
        output = torch.sparse.mm(adjacent, support)  # 稀疏矩阵乘法计算： D-0.5 * (A+I) * D-0.5 * H * W
        if self.bias:  # 加入偏置
...
class GCN(nn.Module):  # 使用定义好的图卷积层搭建三层网络并执行前向传播；
    ...
    def forward(self, input_feature, adjacent):  # 连接网络并前向传播
        # 不需要使用forward，只要在实例化一个对象中传入对应的参数就可以自动调用 forward 函数
        # 在实例化一个对象之后，仅需传入对应参数，无需使用gcn1.forward(input_feature, adjacent)接可以实现对forward函数的调用
        h = F.relu(self.gcn1(adjacent, input_feature))  # 定义中间隐藏层
        h = F.relu(self.gcn2(adjacent, h))
        h = F.relu(self.gcn3(adjacent, h))
