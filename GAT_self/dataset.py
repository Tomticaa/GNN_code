...
IDE     ：PyCharm
File    ：dataset.py
Date    ：2024/7/9 下午3:16
Project ：GNN_code
Project Description：
    对 cora 数据集进行处理，返回 tensor 形式的：adj, features, labels, idx_train, idx_val, idx_test;
    * 引入随机mask进行数据集划分
    * 进行文件序列化，加快模型处理速度；
"""
import torch
...
def process_data(path="../Dataset/cora/", dataset="cora"):
    ...
    return Data(adj=adj, features=features, labels=labels, idx_train=idx_train, idx_val=idx_val, idx_test=idx_test)


def load_data(path="../Dataset/cora/cora_proceed/"):  # 装装载数据

    save_file = osp.join(path, "Data_cora_for_GAT.pkl")  # ./Dataset/cora/processed_cora_for_GAT.pkl
