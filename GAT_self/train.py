...
from GAT_self.dataset import load_data
...
def plot_loss_with_acc(train_loss_history, train_acc_history, val_loss_history, val_acc_history):
    ...
    plt.legend(loc=0)
    plt.title('loss')
    plt.savefig("./assets/img/loss.png")
    plt.show()
    ...
    plt.legend(loc=0)
    plt.title('accuracy')
    plt.savefig("./assets/img/acc.png")
    plt.show()
...
def tsne_visualize():  # 节点聚类可视化
    ...
    plt.legend(loc=0)
    plt.savefig('./assets/img/tsne.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    tsne_visualize()
    torch.save(model, './assets/model/GAT_cora')
    """
